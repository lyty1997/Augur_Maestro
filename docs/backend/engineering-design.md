...
本文档定义 Augur_Maestro M1 的工程落地方案。M1 的目标不是做完整量化平台，而是先跑通 A 股主题股票池的历史数据导入、标准化落库、质量检查、最小回测验证、归因分析和报告闭环。

上位路线见 [technical-roadmap.md](../project-progress/technical-roadmap.md)，历史数据源专项路线见 [data-source-roadmap.md](data/data-source-roadmap.md)。

M1 的具体 PostgreSQL 表、字段、约束和迁移顺序见 [data-model.md](data/data-model.md)。

M1 的 Python DTO、Protocol、服务接口和 CLI/API 调用边界见 [kernel-interfaces.md](kernel/kernel-interfaces.md)。
...
Augur_Maestro M1 使用 PostgreSQL，原因是：
...
本节只保留 M1 股票池相关表的摘要。完整数据模型见 [data-model.md](data/data-model.md)。
...
augur_maestro data init-db --help
augur_maestro data import-symbols --help
augur_maestro data import-daily-bars --help
augur_maestro data validate --help
augur_maestro research --help
augur_maestro research run-backtest --help
augur_maestro research analyze-attribution --help
augur_maestro universe --help
augur_maestro universe build-candidates --help
augur_maestro universe approve --help
```
...
- 两者都能读取 Augur_Maestro 标准化日线数据。
- 能运行一个简单日频策略。
- 能输出收益、回撤、换手率等基础指标。
- 能输出基础归因结果，至少覆盖标的贡献、主题贡献、交易成本贡献和主要回撤片段。
- 能把结果转成 Augur_Maestro Markdown 报告。
...
接口类按 `records.py`、`ports.py`、`services.py` 拆分：`records.py` 放 kernel 对外传递的 DTO，`ports.py` 放外部依赖和仓储 Protocol，`services.py` 放业务编排服务。具体接口定义见 [kernel-interfaces.md](kernel/kernel-interfaces.md)。
...
- `src/backend`：FastAPI 接口层，不写核心研究逻辑。
- `src/cli`：命令行入口，只调用 kernel 暴露的服务。

约束：

- kernel 之间通过接口或数据模型协作，避免互相直接读写内部细节。
- `src/backend` 和 `src/cli` 都是入口层，不能沉淀核心业务规则。
...
- 能在本地启动 PostgreSQL。
- 能通过 CLI 初始化数据库。
- 能按 [kernel-interfaces.md](kernel/kernel-interfaces.md) 建立 M1 kernel 接口类，并保持 `rq_core` 不依赖 FastAPI、Typer、SQLAlchemy、AKShare、Qlib 或 VectorBT。
