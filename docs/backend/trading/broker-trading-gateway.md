...
- 亏损触发策略分为 `risk_warn`、`risk_halt`、`risk_exit` / `risk_hedge`：达到阈值后禁止普通开仓和加仓，暂停自动交易；允许人工确认后的止损、减仓、平仓、补仓建议评审、对冲或做空处置，且不能绕过 OMS、风控、交易时间、额度、审计和白名单。
