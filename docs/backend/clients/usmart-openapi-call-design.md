...
- `finalStateFlag` 与 `status` / `orderStatus` 冲突、缺失或出现未知值时，`BrokerOrderMappingStatus` 进入 `raw_status_conflict`、`raw_status_missing`、`raw_status_unknown` 或 `unknown_by_official_doc`，OMS 订单进入 `unknown`，需要人工或后续查询确认。
- `orderStatus` 是订单明细历史节点状态，必须进入 OMS mapper；完整枚举未覆盖前，映射原因进入 `unknown_by_official_doc`，OMS 对外状态仍是 `unknown`。
