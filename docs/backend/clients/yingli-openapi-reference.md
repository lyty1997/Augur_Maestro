...
- 官方网页文档已给出测试环境地址；用户已确认其可用于非生产联调。本轮只允许真实登录和只读查询，不运行真实下单、改单、撤单。
- 官方网页文档没有明确说明的改单语义、订单状态和错误码，统一标记为 `unknown_by_official_doc`。
...
- 官方测试环境已按用户确认作为非生产联调环境；本轮仍只开放真实登录和只读查询，交易动作继续 dry-run。
