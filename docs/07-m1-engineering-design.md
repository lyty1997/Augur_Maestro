...
上位路线见 [05-technical-roadmap.md](./05-technical-roadmap.md)，历史数据源专项路线见 [06-data-source-roadmap.md](./06-data-source-roadmap.md)。
...
- M1 可以前置 AI 辅助选股，但只用于生成候选股票池，不能直接进入交易。
...
- 第一版不要求用户一只只股票手工输入。可以用 AKShare 概念板块、行业分类、公开资料和 AI 辅助生成候选清单，再由用户确认后写入正式股票池。
- AI 辅助选股只能生成候选池，必须输出候选理由、来源、主题归属和置信度，不能自动加入实盘交易链路。
