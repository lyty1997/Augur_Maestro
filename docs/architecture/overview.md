...
- 官方网页手册已给出 UAT 测试地址，但未明确 UAT 交易动作安全语义；改单语义、订单状态或错误码等未明确项统一标记为 `unknown_by_official_doc`。不得写入真实账号、密钥、token 或资金隐私数据。
...
- 正式接入 miniQMT 和盈立 OpenAPI；其中盈立需先完成申请、官方网页手册字段确认、UAT 是否保证不产生真实委托的确认和安全设计确认。
- 实现统一账户视图。
- 实现多市场交易日历和时间处理。
- 实现自动对账和异常告警。
- 扩展条件单、止盈止损、触发单、分批下单。
