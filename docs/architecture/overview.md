...
Augur_Maestro 是一个面向个人投资者和个人研究者的低频量化交易系统。长期计划同时接入：

- miniQMT：主要用于对接国内券商 QMT 体系，例如东北证券账户，覆盖 A 股相关交易和行情能力。当前已确认 miniQMT 支持无人值守运行。
- 盈立证券 OpenAPI：用于港股、美股账户的行情、交易、撤单、持仓和资金查询。当前 API 申请结果不确定；官方真相源已切换为 uSmart 网页手册和本地 Markdown 转换稿，官方测试环境接口地址已按用户确认作为非生产联调环境；正式实现前仍需逐条确认认证、endpoint、订单类型和状态码，不作为 M1 研究闭环依赖。
...
M1 的当前实现口径更窄：先用本地 PostgreSQL、Typer CLI、AKShare 启动数据源、第二数据源交叉验证、Qlib/VectorBT 并行最小回测验证和 Markdown 报告，跑通 A 股主题股票池的研究回测闭环。FastAPI 和 React 保留为技术栈方向，但 M1 的主操作入口先是 CLI；真实账户、真实订单、真实持仓、真实资金、真实对账和真实券商下单都不进入 M1。

券商接入的硬边界必须提前定清楚：miniQMT 和盈立交易开放 API 的登录、账户、持仓、订单、成交等只读查询测试，以及盈立基础报价 API / 报价推送 API 的行情只读查询测试，不构成真实交易，但它们走不同网关和不同协议。任何会向券商提交委托、撤单、条件单、预埋单、止盈止损、触发单或其他可能在券商侧形成订单的调用，都按真实交易处理。所有真实交易能力默认关闭，必须经过实盘开关、人工确认、交易时间检查、风控、OMS 和对账设计后才能启用。
...
- 股票池只导入用户熟悉主题相关标的，不先导入全 A 股；AI 可以扩展候选清单，但必须人工确认后才能进入正式研究股票池。
- 正式研究股票池需要过滤 ST、长期停牌、低成交额、低换手率、成交稀疏和疑似容易被大资金操纵的小票股。
- 主题范围包括算力、电力、半导体、人工智能、芯片、电池、通信、银行、CPO、机器人、存储、有色金属、影视院线。
- 历史数据默认导入最近 10 年；最近 1 年作为模拟数据，此前数据按时间顺序 8:2 切分为建模数据和回测数据。
- 研究数据源默认使用 AKShare，但必须通过 `DataProvider` 抽象写入内部标准表；同时引入第二数据源做关键字段抽样交叉验证，研究和回测不能直接调用外部数据源 SDK。
- 使用成熟回测库做执行引擎，并行验证 Qlib 和 VectorBT，不自研完整回测框架。
...
- 长期支持统一券商交易网关，后续优先确认 miniQMT，再根据盈立 OpenAPI 申请和 uSmart 官方网页手册决定港股、美股接入。
...
- M1 不接 miniQMT 真实交易，也不接盈立 OpenAPI 正式交易。盈立 OpenAPI 申请所需源码截图属于券商申请材料事项，不作为 Augur_Maestro 开发交付；开发侧只按 uSmart 官方网页手册和本地 Markdown 转换稿做接口设计，不允许真实提交下单、改单或撤单请求。
...
- M1 必须同时引入第二数据源，用于抽样校验收盘价、成交量、成交额、停牌/缺失等关键字段；第二源第一版选择 BaoStock。
...
- `BaostockDataProvider`：M1 第一版交叉校验源，只用于数据质量校验和差异提示，不改变 AKShare 主数据写入路径。
- `TushareDataProvider` 或本地导入 Provider：作为后续备用校验源。
...
M1 回测引擎优先采用成熟开源库，不自研完整回测框架。Augur_Maestro 自己负责统一数据模型、实验记录、风控假设、报告格式和人工审核流程；成熟回测库负责执行历史回测，避免过早造轮子。
...
- `cancel_pending`：券商侧等待撤单。
- `cancelled`：已撤单。
- `cancel_rejected`：券商拒绝撤单，原订单继续跟踪。
- `modify_requested` / `modify_pending` / `modify_rejected`：改单请求、等待改单和改单拒绝。
- `partial_cancelled`：部分成交后剩余数量已撤销。
- `broker_rejected`：券商拒单。
- `failed`：本地确认未形成有效券商委托的失败。
- `unknown`：状态未知，必须查询、对账或人工确认。
...
M1 不实现真实 `MiniQMTTradingAdapter` 或真实交易 transport。2026-05-27 用户确认旧 `broker_kernel`、旧 uSmart adapter/client 和旧 `TradingGateway` 测试均为未经确认草稿，已经删除；后续 broker 模块必须按已确认 DTO、profile、错误码、审计和 raw record 设计重新实现。具体券商 SDK 行为必须等权限、运行环境、uSmart 官方网页手册和安全设计确认后再启用。

券商适配器硬约束：

- 只读接入中，交易开放 API 只允许登录、权限检查、账户查询、资金查询、持仓查询、订单查询和成交查询；行情只读查询走基础报价 API 或报价推送 API，并由 `QuotationDataGateway` 承接。两者不允许调用任何下单、撤单或条件单接口。
...
- OpenAPI 的协议、频率限制、订单类型、盘前盘后、撤单语义和错误码必须以 uSmart 官方网页手册为准，未确认前不得假设为可交易能力。
- 官方网页手册已给出官方测试环境接口地址，用户已确认其可用于非生产联调；本轮只开放真实登录和只读查询，交易动作继续 dry-run。改单语义、订单状态或错误码等未明确项统一标记为 `unknown_by_official_doc`。不得写入真实账号、密钥、token 或资金隐私数据。
...
- 已确认 M1 市场：A 股研究回测闭环。
- 明确券商 API 能力边界；miniQMT 后续优先确认权限、运行环境和接口能力，盈立 OpenAPI 已有官方网页手册、本地 Markdown 转换稿和官方测试环境接口地址，但仍等待申请通过和接口行为联调确认。
...
- 接入 Qlib 和 VectorBT 做并行最小验证，再选择 M1 主执行器或明确分工。
...
- 正式接入 miniQMT 和盈立 OpenAPI；其中盈立需先完成申请、官方网页手册字段确认、官方测试环境联调和安全设计确认。
- 实现统一账户视图。
- 实现多市场交易日历和时间处理。
- 实现自动对账和异常告警。
- 扩展条件单、止盈止损、触发单、分批下单。
...
- AKShare 与 BaoStock、后续 miniQMT 数据源之间的差异校验方案。
...
- 盈立 OpenAPI 官方测试环境联调结果，以及官方网页手册中港股、美股订单类型、盘前盘后、行情权限、频率限制、错误码和订单状态码确认。
