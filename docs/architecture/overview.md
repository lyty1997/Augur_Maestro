...
- 盈立证券 OpenAPI：用于港股、美股账户的行情、交易、撤单、持仓和资金查询。当前 API 申请结果不确定；官方真相源已切换为 uSmart 网页手册和本地 Markdown 转换稿，正式实现前仍需逐条确认认证、endpoint、订单类型、状态码和 sandbox，不作为 M1 研究闭环依赖。
...
- M1 不接 miniQMT 真实交易，也不接盈立 OpenAPI 正式交易。盈立 OpenAPI 申请所需源码截图属于券商申请材料事项，不作为 RobustQuant 开发交付；开发侧只按 uSmart 官方网页手册和本地 Markdown 转换稿做接口设计，不允许真实提交下单、改单或撤单请求。
...
M1 不实现真实 `MiniQMTTradingAdapter` 或真实交易 transport。当前先保留 `BrokerTradingAdapter` 基类和 `TradingGateway` 安全门面；uSmart 适配器可以写出登录、下单、改单、撤单代码外形，但默认由 `TradingGateway` 的 `read_only` 安全模式阻断真实交易动作。具体券商 SDK 行为必须等权限、运行环境、uSmart 官方网页手册和安全设计确认后再启用。
...
- 官方网页手册已给出 UAT 测试地址；用户已确认不自动把 UAT 当作可交易 sandbox。未明确 UAT 交易动作安全语义前，交易动作继续 dry-run；改单语义、订单状态或错误码等未明确项统一标记为 `unknown_by_official_doc`。不得写入真实账号、密钥、token 或资金隐私数据。
...
- 已确认 M1 市场：A 股研究回测闭环。
- 明确券商 API 能力边界；miniQMT 后续优先确认权限、运行环境和接口能力，盈立 OpenAPI 已有官方网页手册、本地 Markdown 转换稿和 UAT 测试地址，但仍等待申请通过、UAT 交易动作安全语义和接口行为联调确认。
...
- 正式接入 miniQMT 和盈立 OpenAPI；其中盈立需先完成申请、官方网页手册字段确认、UAT 是否保证不产生真实委托的券商确认和安全设计确认。
- 实现统一账户视图。
- 实现多市场交易日历和时间处理。
- 实现自动对账和异常告警。
- 扩展条件单、止盈止损、触发单、分批下单。
...
- 盈立 OpenAPI UAT 是否等同 sandbox / paper trading，以及官方网页手册中港股、美股订单类型、盘前盘后、行情权限、频率限制、错误码和订单状态码确认。
