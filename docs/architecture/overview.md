...
- 盈立证券 OpenAPI：用于港股、美股账户的行情、交易、撤单、持仓和资金查询。当前 API 申请结果不确定；用户已提供繁体中文官方 PDF，正式实现前仍需逐条确认认证、endpoint、订单类型、状态码和 sandbox，不作为 M1 研究闭环依赖。
...
- M1 不接 miniQMT 真实交易，也不接盈立 OpenAPI 正式交易。盈立 OpenAPI 申请所需源码截图属于券商申请材料事项，不作为 RobustQuant 开发交付；开发侧只解析官方 PDF，不允许真实提交下单、改单或撤单请求。
...
- PDF 未说明的 sandbox、改单语义、订单状态或错误码，统一标记为 `unknown_by_pdf`；不得写入真实账号、密钥、token 或资金隐私数据。
...
- 已确认 M1 市场：A 股研究回测闭环。
- 明确券商 API 能力边界；miniQMT 后续优先确认权限、运行环境和接口能力，盈立 OpenAPI 已有官方 PDF 但仍等待申请通过、sandbox 和接口行为摘录确认。
...
- 正式接入 miniQMT 和盈立 OpenAPI；其中盈立需先完成申请、官方 PDF 摘录确认、沙箱或测试环境确认和安全设计确认。
- 实现统一账户视图。
- 实现多市场交易日历和时间处理。
- 实现自动对账和异常告警。
- 扩展条件单、止盈止损、触发单、分批下单。
...
- 盈立 OpenAPI sandbox，以及官方 PDF 中港股、美股订单类型、盘前盘后、行情权限、频率限制、错误码和订单状态码摘录。
