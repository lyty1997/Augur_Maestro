...
- 官方网页手册未说明的 sandbox、改单语义、订单状态或错误码，统一标记为 `unknown_by_official_doc`；不得写入真实账号、密钥、token 或资金隐私数据。
...
- 已确认 M1 市场：A 股研究回测闭环。
- 明确券商 API 能力边界；miniQMT 后续优先确认权限、运行环境和接口能力，盈立 OpenAPI 已有官方网页手册和本地 Markdown 转换稿，但仍等待申请通过、sandbox 和接口行为联调确认。
...
- 正式接入 miniQMT 和盈立 OpenAPI；其中盈立需先完成申请、官方网页手册字段确认、沙箱或测试环境确认和安全设计确认。
- 实现统一账户视图。
- 实现多市场交易日历和时间处理。
- 实现自动对账和异常告警。
- 扩展条件单、止盈止损、触发单、分批下单。
...
- 盈立 OpenAPI sandbox，以及官方网页手册中港股、美股订单类型、盘前盘后、行情权限、频率限制、错误码和订单状态码确认。
