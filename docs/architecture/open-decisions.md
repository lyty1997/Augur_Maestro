...
- 第一版 `live_guarded` 风控阈值：单笔普通订单最大金额 `500 USD`，美股碎股单笔最大金额 `100 USD`，单只股票最大持仓比例 `10%`，最大持仓股票数量 `20`，单日最大交易次数 `5`，单日最大成交金额 `2000 USD`，单日最大亏损阈值 `200 USD`。
- 日内亏损达到预警线进入 `risk_warn`：禁止加仓亏损标的，允许止损、减仓、平仓和对冲候选生成。
- 日内亏损达到阈值进入 `risk_halt`：禁止新的普通开仓和加仓，暂停自动交易任务；允许生成补仓、对冲、做空建议，但不能自动执行。
- 人工确认后可进入 `risk_exit` / `risk_hedge`：允许止损、减仓、平仓，以及按风控批准的对冲或做空标的；仍必须经过 OMS、风控、交易时间、额度、审计和白名单。
