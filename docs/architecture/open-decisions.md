...
- 真实交易能力默认关闭，必须经过实盘开关、策略授权、交易时间检查、风控、OMS、账户/标的白名单和对账设计后才能启用；运行期基础交易不要求逐笔人工确认，自动化程序可以执行通过风控门禁的建仓、加仓、减仓、平仓和止损。
...
- 第一版只做现金账户多头交易，允许自动化程序按目标仓位执行建仓、加仓、减仓、平仓和止损；不启用自动对冲、做空、融资融券、保证金交易或美股期权。
- 初期只允许限价单，不允许市价单。
...
- 第一版只做现金账户多头交易；自动对冲、做空、融资融券、保证金交易和美股期权全部后置到后续迭代。
- 第一批交易白名单先不固定，等待量化研究 Agent 输出候选标的。
- 模拟盘阶段可以更自由地覆盖个股、行业 ETF 和主题股票池。
- 第一版 `live_guarded` 风控阈值：单笔普通订单最大金额 `500 USD`，美股碎股单笔最大金额 `100 USD`，单只股票最大持仓比例 `10%`，最大持仓股票数量 `20`，单日最大交易次数 `5`，单日最大成交金额 `2000 USD`，单日最大亏损阈值 `200 USD`。
- 日内亏损达到预警线进入 `risk_warn`：进入更严格的仓位控制模式，自动建仓、加仓、减仓、平仓和止损必须符合目标仓位、损失预算、单票/单日额度和白名单；禁止无目标仓位依据的追涨、摊平或绕过风控。
- 日内亏损达到阈值进入 `risk_halt`：暂停非必要扩张型策略，但不关闭无人值守处置能力；已授权策略若给出目标仓位再平衡意图，建仓、加仓、减仓、平仓和止损仍可在加强风控门禁通过后自动执行。
- 系统可进入 `risk_rebalance` / `risk_exit`：在风控门禁通过后自动执行目标仓位调整，包括建仓、加仓、减仓、平仓和止损；`risk_hedge`、做空、融资融券、保证金交易和美股期权后置到后续迭代，需等待白名单、权限、额度和高风险授权确认后再启用。
...
- 用户确认第一版先按现金账户多头交易设计，不启用自动对冲、做空、融资融券、保证金交易或美股期权；后续迭代再加入对冲、做空和美股期权。
