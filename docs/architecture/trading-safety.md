...
- 风控通过。
- OMS 状态合法。
- 实盘开关和策略授权要求已满足；已授权策略的建仓、加仓、减仓、平仓和止损不要求逐笔人工确认。
- 对账状态正常。
...
- 亏损阈值触发后不硬抗，也不无依据追损；进入 `risk_warn` / `risk_halt` 后改用更严格的仓位控制门禁。已授权策略的目标仓位再平衡可以无人值守执行建仓、加仓、减仓、平仓和止损；第一版不自动执行对冲、做空、融资融券、保证金交易或美股期权，后续迭代需单独确认白名单、权限、额度和高风险授权流程。
- 第一批实盘宽基 ETF 白名单。
