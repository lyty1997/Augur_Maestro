...
本文档是 Augur_Maestro 交易安全边界的专项真相源。它定义后续 M2/M3 进入模拟盘和实盘前必须满足的规则。
...
盈立 OpenAPI 申请所需源码截图属于券商申请材料事项，不作为 Augur_Maestro 开发交付，也不等同于上述三种运行模式。开发侧只按 uSmart 官方网页手册和本地 Markdown 转换稿做接口设计；任何真实下单、改单、撤单请求仍必须按真实交易能力处理。
...
- 风控通过。
- OMS 状态合法。
- 实盘开关和策略授权要求已满足；已授权策略的建仓、加仓、减仓、平仓、止盈和止损不要求逐笔人工确认。
- 对账状态正常。
...
- 禁止交易名单。
- 交易时间。
- 单笔订单金额上限：第一版 `live_guarded_small` 普通订单 `min(500 USD, 5% NAV)`，美股碎股 `min(100 USD, 1% NAV)`。
- 单只股票最大持仓比例：第一版 `10% NAV`。
- 最大持仓股票数量：第一版 `20`。
- 单日最大交易次数：第一版 `5`。
- 单日最大成交金额：第一版 `min(2000 USD, 20% NAV)`。
- 单日最大亏损阈值：第一版 `min(200 USD, 2% NAV)`，触发后进入风险处置模式。
- 可用资金或可卖数量。
- 价格偏离限制。
- 订单类型是否允许。
- 重大事件窗口限制。
...
第一版不使用券商侧条件单、券商侧止盈止损、触发单、预埋单或分批下单。自动止盈止损由策略模块产生普通正股买卖意图，触发后仍然必须经过 OMS、风控、交易时间、白名单、额度、审计和对账链路。
...
后续进入 M3 受控实盘时，第一批交易白名单由量化研究 Agent 输出候选标的，再经过人工批准发布，不固定写死宽基 ETF 清单。

白名单可以包含美股正股和 ETF，但第一版默认偏向高流动性、低价差标的，用小资金验证订单链路、风控、OMS 和对账。

默认规则：

- 模拟盘可以覆盖主题股票池、行业 ETF、宽基 ETF 和个股。
- 实盘白名单必须人工发布，且每个条目至少包含 `symbol`、`market=US`、`asset_type=stock|etf`、`max_position_pct`、`max_order_notional`、`enabled_strategy_ids`、`approved_at`、`expires_at`。
- `TradingGateway` 不选股，只检查标的是否在当前有效白名单内；无有效白名单命中时，OMS 阻断交易并返回 `risk.symbol_not_whitelisted`。
- 即使在白名单内，也必须经过交易时间、风控、OMS、对账和人工开关。
...
- 可以在官方测试环境中验证；用户已确认该测试环境用于非生产联调。
- 用户已确认项目策略：官方测试环境接口地址可用于非生产联调；本轮只允许真实登录和只读查询，不运行真实下单、改单、撤单。
- 不能为了源码截图材料编写或运行真实交易代码。
- 不能为了申请材料绕过风控、OMS 和实盘授权规则。
...
- 第一版实盘是否只允许限价单。
- 第一版风控阈值已按 `live_guarded_small` 确定；阈值配置化，实际生效值按固定上限与账户净值 `NAV` 比例取更保守值。
- 亏损阈值触发后不硬抗，也不无依据追损；进入 `risk_warn` / `risk_halt` 后改用更严格的仓位控制门禁。已授权策略的目标仓位再平衡、止盈和止损可以无人值守产生普通正股买卖意图；`TradingGateway` 只执行通过 OMS 和风控的正股买入卖出，不内置止盈止损判断。第一版不自动执行对冲、做空、融资融券、保证金交易或美股期权，后续迭代需单独确认白名单、权限、额度和高风险授权流程。
- 第一批实盘白名单由量化研究 Agent 输出候选并经人工批准发布；`TradingGateway` 只做有效白名单检查。
