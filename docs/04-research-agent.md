...
研究回测 Agent 是 RobustQuant 内嵌的自动研究助理。它负责把“我想研究一个策略方向”拆解成一系列可执行任务，并自动完成数据准备、选股、因子挖掘、训练、回测、模拟和报告输出。

它不是实盘交易 Agent。它不能直接下单，不能绕过人工审核，不能把候选策略直接推到实盘。

第一阶段已确认先做 A 股研究回测闭环。券商交易 API 尚未齐备，因此研究回测 Agent 是当前最适合优先推进的主线。它可以先在历史数据和模拟环境中把研究闭环跑起来，等 miniQMT 等券商接口条件成熟后，再把通过审核的策略接入交易链路。盈立 OpenAPI 申请结果不确定，不作为第一阶段依赖。第一阶段历史行情默认通过 AKShare 导入 PostgreSQL 标准表，项目总体路线见 [05-technical-roadmap.md](./05-technical-roadmap.md)，数据源专项路线见 [06-data-source-roadmap.md](./06-data-source-roadmap.md)。
...
第一阶段建议支持配置化输入，后续再支持自然语言输入。第一阶段市场固定为 A 股，避免同时处理多市场交易日历、币种、税费和交易规则带来的复杂度。
...
建议第一阶段使用 walk-forward，也叫滚动窗口验证。简单理解：
...
第一阶段建议优先生成策略配置，少生成代码。配置更安全，也更容易审核。

### 5.8 回测执行器

回测执行器负责验证候选策略在历史上表现如何。

第一阶段优先接入成熟回测库作为执行引擎，不自研完整回测框架。RobustQuant 自己保留数据标准化、实验记录、研究报告、人工审核和后续实盘风控边界；回测库负责历史回测计算。具体选型后续需要比较 A 股适配能力、维护活跃度、性能、事件处理、手续费/滑点/涨跌停/停牌支持，以及是否容易接入自定义数据。
...
- 事件发布后直接自动下单。
- 利用快速事件链路自动加仓或追涨。
- 条件单、止盈止损、触发单、分批下单等高级订单能力；这些需要在后续 OMS 和券商适配器设计中预留，但不进入第一阶段研究回测实现。

## 10. 待确认问题

- 第一阶段先导入全 A 股，还是先导入 100 到 500 只样本股。
- 成熟回测库具体选型：Qlib、Backtrader、VectorBT、Zipline/Zipline Reloaded，或其他更适合 A 股的方案。
- Agent 是否第一阶段就接 LLM。
- 报告是否需要 HTML/PDF，还是先 Markdown。
- 模拟盘是否需要实时行情，还是先离线 replay。
- 消息面数据源选哪个，是否能稳定提供事件预期值、公布值和准确发布时间。
- 事件信号先用于报告观察，还是第一阶段就注入策略回测。
