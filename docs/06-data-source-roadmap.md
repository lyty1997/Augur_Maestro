...
本文档定义 RobustQuant 第一阶段 A 股研究回测闭环所需的历史数据源路线。核心目标是：先用足够轻量的数据源把研究、因子、回测和报告跑起来，同时保证未来可以切换到 miniQMT 或其他券商数据源，而不重写策略和回测逻辑。

第一阶段的数据目标不是追求全市场、全字段、全频率，而是建立一条可复现、可替换、可校验的研究数据链路。

## 2. 已确认决策

- 第一阶段研究市场为 A 股。
- 第一阶段默认历史行情启动源使用 AKShare。
...
因此第一阶段可以用 AKShare 启动，但工程设计必须默认“未来会替换或并行校验数据源”。
...
6. 写入完成后更新导入批次状态。
7. 研究 Agent 和回测模块只读取 PostgreSQL 标准表。
...
- 接入 BaoStock、Tushare 或本地导入中的至少一个作为校验源。
- 对关键字段做抽样对比。
- 记录数据差异报告。
...
- 第一阶段是否先只导入日线，分钟线延后。
- 第一阶段股票池是先导入全 A 股，还是先选 100 到 500 只样本股。
- 成熟回测库选型是否优先考虑 Qlib、Backtrader、VectorBT 或其他方案。
- PostgreSQL 是否本地 Docker 部署，还是直接安装本机服务。
